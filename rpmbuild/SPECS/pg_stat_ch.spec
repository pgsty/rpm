...
%prep
%setup -q -n pg_stat_ch-src

patch -p1 --forward -f < %{_specdir}/patches/pg_stat_ch-0.3.3-use-system-grpc-absl-and-std-unordered-map.patch

%build
git config --global http.version HTTP/1.1
cmake -B build -G Ninja \
  -DPG_CONFIG=%{pginstdir}/bin/pg_config \
  -DOTELCPP_PROTO_PATH=%{_builddir}/pg_stat_ch-src/third_party/opentelemetry-cpp/third_party/opentelemetry-proto \
